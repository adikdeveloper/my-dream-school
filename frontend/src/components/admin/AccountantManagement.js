...
const AccountantManagement = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setLoading(true);
      const res = await apiService.getUsers('accountant', 1, 100);
      setAccountants(res.users || []);
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = { ...form, username: form.phone, role: 'accountant' };
      if (modal === 'add') {
        await apiService.createUser(payload);
        showToast("Hisobchi muvaffaqiyatli qo'shildi!");
      } else {
        if (!payload.password) delete payload.password;
        await apiService.updateUser(selected._id, payload);
        showToast("Hisobchi ma'lumotlari yangilandi!");
      }
      closeModal();
      await load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast("Hisobchi o'chirildi!", 'error');
      closeModal();
      await load();
    } catch (e) {
  ...
  const filtered = useMemo(() => accountants.filter(a => {
    ...
    return match;
  }), [accountants, debouncedSearch, filter]);

  const stats = useMemo(() => ({
    total: accountants.length,
    active: accountants.filter(a => a.isActive).length,
    inactive: accountants.filter(a => !a.isActive).length,
  }), [accountants]);
  ...
  return (
    <div className="teacher-management">
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)' }}>
        {[{ icon: '💰', val: stats.total, label: 'Jami hisobchilar', cls: 'stat-total' }, { icon: '✅', val: stats.active, label: 'Faol', cls: 'stat-active' }, { icon: '⏸️', val: stats.inactive, label: 'Nofaol', cls: 'stat-inactive' }].map((s, i) => (
      ...
      <div className="filters-container">
        <div className="filter-card">
          ...
          <button className="btn-add-compact" onClick={openAdd}>
            ...
            <span className="add-text">Yangi hisobchi</span>
          </button>
        </div>
      </div>
        ...
        {isLoadingData ? (
          ...
          <div className="table-view">
            <table className="teachers-table">
              <thead>
                <tr><th>Hisobchi (F.I.SH)</th><th>Telefon</th><th>Passport</th><th>Holat</th><th>Amaliyotlar</th></tr>
              </thead>
              <tbody>
                {filtered.map(acc => (
          ...
          <div className="empty-state">
            ...
            <h3 className="empty-title">Hisobchilar topilmadi</h3>
            <p className="empty-text">Qidiruv kriteriyalaringizga mos hisobchi mavjud emas.</p>
          </div>
        )}
      </div>
          ...
          <div className="am-modal" onClick={e => e.stopPropagation()}>
            <div className="am-modal-header">
              <div className="am-modal-title">
                ...
                {modal === 'add' ? "Yangi hisobchi qo'shish" : "Hisobchini tahrirlash"}
              </div>
        ...
        <div className="am-overlay" onClick={closeModal}>
          <div className="am-modal am-modal-view" onClick={e => e.stopPropagation()}>
            <div className="am-modal-header am-header-view">
              <div className="am-modal-title"><span className="am-modal-icon">👁️</span>Hisobchi ma'lumotlari</div>
              <button className="am-close-btn" onClick={closeModal}>✕</button>
            </div>
            <div className="am-modal-body">
        ...
        <div className="am-overlay" onClick={closeModal}>
          <div className="am-modal am-modal-delete" onClick={e => e.stopPropagation()}>
            <div className="am-delete-body">
              ...
              <h3 className="am-delete-title">Hisobchini o'chirish</h3>
