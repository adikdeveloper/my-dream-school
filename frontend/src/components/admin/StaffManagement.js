...
// Har bir xodim turi uchun rol va matnlar. Route :type qiymatiga mos keladi.
const STAFF_CONFIG = {
  admin:         { role: 'admin',      icon: '🛡️', singular: 'Admin',               plural: 'Adminlar' },
  accountant:    { role: 'accountant', icon: '💰', singular: 'Hisobchi',            plural: 'Hisobchilar' },
...
const StaffManagement = () => {
  ...
  const cfg = STAFF_CONFIG[type] || DEFAULT_CFG;
  ...
  const load = useCallback(async () => {
    if (!cfg.role) { setStaffList([]); return; }
    try {
      setIsLoadingData(true);
      setLoading(true);
      const res = await apiService.getUsers(cfg.role, 1, 100);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = { ...form, username: form.phone, role: cfg.role };
      if (modal === 'add') {
        await apiService.createUser(payload);
        showToast(`${cfg.singular} muvaffaqiyatli qo'shildi!`);
      } else {
        if (!payload.password) delete payload.password;
        await apiService.updateUser(selected._id, payload);
        showToast(`${cfg.singular} ma'lumotlari yangilandi!`);
      }
      closeModal();
      await load();
    } catch (e) {
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast(`${cfg.singular} o'chirildi!`, 'error');
      closeModal();
      await load();
    } catch (e) {
  ...
  const filtered = useMemo(() => staffList.filter(a => {
    ...
    return match;
  }), [staffList, debouncedSearch, filter]);

  const stats = useMemo(() => ({
    total: staffList.length,
    active: staffList.filter(a => a.isActive).length,
    inactive: staffList.filter(a => !a.isActive).length,
  }), [staffList]);
  ...
  return (
    <div className="teacher-management">
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)' }}>
        {[{ icon: cfg.icon, val: stats.total, label: `Jami ${cfg.plural.toLowerCase()}`, cls: 'stat-total' }, { icon: '✅', val: stats.active, label: 'Faol', cls: 'stat-active' }, { icon: '⏸️', val: stats.inactive, label: 'Nofaol', cls: 'stat-inactive' }].map((s, i) => (
      ...
      <div className="filters-container">
        <div className="filter-card">
          ...
          <button className="btn-add-compact" onClick={openAdd}>
            ...
            <span className="add-text">Yangi {cfg.singular}</span>
          </button>
        </div>
      </div>
        ...
        {isLoadingData ? (
          ...
          <div className="table-view">
            <table className="teachers-table">
              <thead>
                <tr><th>{cfg.singular} (F.I.SH)</th><th>Telefon</th><th>Passport</th><th>Holat</th><th>Amaliyotlar</th></tr>
              </thead>
              <tbody>
                {filtered.map(acc => (
          ...
          <div className="empty-state">
            ...
            <h3 className="empty-title">{cfg.plural} topilmadi</h3>
            <p className="empty-text">Qidiruv kriteriyalaringizga mos {cfg.singular.toLowerCase()} mavjud emas.</p>
          </div>
        )}
      </div>
          ...
          <div className="am-modal" onClick={e => e.stopPropagation()}>
            <div className="am-modal-header">
              <div className="am-modal-title">
                ...
                {modal === 'add' ? `Yangi ${cfg.singular} qo'shish` : `${cfg.singular}ni tahrirlash`}
              </div>
        ...
        <div className="am-overlay" onClick={closeModal}>
          <div className="am-modal am-modal-view" onClick={e => e.stopPropagation()}>
            <div className="am-modal-header am-header-view">
              <div className="am-modal-title"><span className="am-modal-icon">👁️</span>{cfg.singular} ma'lumotlari</div>
              <button className="am-close-btn" onClick={closeModal}>✕</button>
            </div>
            <div className="am-modal-body">
        ...
        <div className="am-overlay" onClick={closeModal}>
          <div className="am-modal am-modal-delete" onClick={e => e.stopPropagation()}>
            <div className="am-delete-body">
              ...
              <h3 className="am-delete-title">{cfg.singular}ni o'chirish</h3>
